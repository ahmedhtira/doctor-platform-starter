-- M4: book_appointment/reschedule_appointment validated working hours,
-- breaks, and blocked periods via private.is_within_working_window, but
-- never checked minimum booking notice or past times — only the *read*
-- path (public.compute_available_slots) did. A client could submit a
-- past or too-soon starts_at and it would be accepted. Fixed by adding a
-- p_now parameter to is_within_working_window and rejecting
-- starts_at < now + doctor's min_booking_notice_minutes, matching the
-- read path's own notice logic.

-- Signature is changing (new trailing param) — CREATE OR REPLACE would
-- otherwise create a second overload alongside the old 4-arg version
-- instead of replacing it.
drop function if exists private.is_within_working_window(uuid, uuid, timestamptz, timestamptz);

create function private.is_within_working_window(
  p_doctor_id uuid,
  p_clinic_id uuid,
...
begin
  ...
  if p_starts_at < p_now + make_interval(mins => coalesce(v_notice_minutes, 0)) then
    return false;
  end if;
...
revoke all on function private.is_within_working_window(uuid, uuid, timestamptz, timestamptz, timestamptz) from public, anon, authenticated, service_role;
